...
`ENABLED_CHAINS` selects which chains this instance indexes and relays for.
All chain addresses, deployment blocks, and cbChainIds are baked into
`src/chains/registry.ts` (not env vars) — update the registry after each
new deployment. Each enabled chain needs a corresponding `RPC_<SLUG>` URL.

| Name                 | Roles requiring it | Default | Format / notes    |
| --------------------- | ------------------- | ------- | ------------------- |
| `ENABLED_CHAINS`      | all                  | -       | Comma-separated registry slugs, e.g. `arcmainnet` or `anvil,solanadevnet`. Validation rejects: unknown slugs, slugs with no deployed diamond address / program id, and slugs with no matching `RPC_<SLUG>` var. All problems are reported together at boot. |
| `RPC_ARCMAINNET`      | if `arcmainnet` enabled | -  | https URL for Arc Mainnet RPC. |
| `RPC_ANVIL`           | if `anvil` enabled   | -       | https / http URL for the local Anvil node (e.g. `http://127.0.0.1:8545`). |
| `RPC_SOLANADEVNET`    | if `solanadevnet` enabled | - | https URL for Solana Devnet RPC (e.g. `https://api.devnet.solana.com`). |

## Relayer keys

The EVM key needs gas plus `RELAYER_ROLE` on every deployed EVM diamond (see
