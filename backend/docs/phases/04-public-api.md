...
**Branch:** `backend-v2-04-public-api` (worktree from `main`) → merged locally into `main` after review
**Depends on:** phases 2a, 2b, 3a merged into `main`
...
1. `src/api/`: controllers + services for every endpoint in SPEC §12.2 not
   ...
   (pagination, amount objects including `requestedAmount` / `amount` for
   payments and `amount` / `fee` / `netAmount` for withdrawals, chain objects,
   address formatting, errors).
2. `PUT /payables/:id/description`: authenticated; host check against the
   indexed payable, or on-chain (the diamond's `isPayableHost(payableId, account)` / Solana
   payable account) when not yet indexed; sanitise to plain text; 3–3000 chars;
