...
contract CbPayload {
  ...
  function encodeStartPaymentPayload(
    StartPaymentPayload memory payload
  ...
  function decodeStartPaymentPayload(
    bytes memory encoded
  ) public pure returns (StartPaymentPayload memory parsed) {
    uint256 index;
  ...
  /// @return encoded bytes
  function encodeCompletePaymentPayload(
    CompletePaymentPayload memory payload
  ) public pure returns (bytes memory encoded) {
  ...
  function decodeCompletePaymentPayload(
    bytes memory encoded
  ) public pure returns (CompletePaymentPayload memory parsed) {
    uint256 index;
    (parsed.payableId, index) = encoded.asBytes32(0);
