...
contract CbTransactions is CbUtils {
  ...
  function _checkCircleMessage(
    ...
    // Body: amount at 148 + 68 = 216, feeExecuted at 148 + 164 = 312.
    uint256 burnAmount;
    (burnAmount,) = circleBridgeMessage.asUint256(216);
    uint256 feeExecuted;
    (feeExecuted,) = circleBridgeMessage.asUint256(312);
    if (burnAmount - feeExecuted < expectedAmount) revert CircleAmountMismatch();
  }
  ...
  /// @param token The address of the token that was paid.
  /// @param amount The amount paid.
  /// @param payerPaymentId The ID of the UserPayment record on the source chain.
    ...
    returns (bytes32 userPaymentId, bytes32 payablePaymentId)
  {
    ...
    payablePaymentId =
      _recordPayablePayment(payableId, toWormholeFormat(msg.sender), config.cbChainId, token, amount, userPaymentId);
    ...
    returns (bytes32 userPaymentId, uint64 wormholeMessageSequence)
  {
    ...
    // Guard: total burn (amount + maxFee) must fit in uint64 for cross-chain payload encoding.
    if (amount + maxFee > type(uint64).max) revert AmountExceedsCrossChainLimit();
    ...
    /* STATE CHANGES */
    // Record successful payment and activity from the payer.
    // Record amount + maxFee (the actual burn amount taken from the payer's wallet).
    ...
    bytes memory encodedPayload = PaymentPayload({
        ...
        actionType: 5,
        payableId: payableId,
        nonce: SafeCast.toUint64(users[msg.sender].paymentsCount),
    ...
    /* INTERACTIONS */
    // Pull amount + maxFee from payer: amount is the payment, maxFee covers Circle's fast fee.
    IERC20(token).safeTransferFrom(msg.sender, address(this), amount + maxFee);

    // Approve the Circle Bridge for the full burn amount.
    SafeERC20.safeIncreaseAllowance(IERC20(token), config.circleBridge, amount + maxFee);
    ...
    // Burn tokens via Circle CCTP with fast finality (threshold=1000).
    circleBridge().depositForBurn(amount + maxFee, destDomain, destContract, token, destContract, maxFee, 1000);
    ...
    if (hasWormhole() && supportsWormhole(_payable.chainId)) {
      ...
      circleTransmitter().sendMessage(destDomain, destContract, destContract, 1000, encodedPayload);
  ...
  /// @return payablePaymentId The ID of the recorded payment from the payable.
  function receiveForeignPaymentViaCctp(RedeemCirclePaymentParameters memory params)
    public
    returns (bytes32 payablePaymentId)
  {
    if (params.wormholeEncoded.length > 0) {
      ...
      if (consumedPaymentNonces[payload.payerChainId][payload.payer][payload.nonce]) revert HasAlreadyConsumedMessage();
      consumedPaymentNonces[payload.payerChainId][payload.payer][payload.nonce] = true;

      payablePaymentId =
        _recordPayablePayment(payableId, payload.payer, payload.payerChainId, token, amount, payload.payerPaymentId);
      ...
      // Validate no trailing bytes in the message (148-byte header + 251-byte payload).
      if (params.circlePayloadMessage.length != 399) revert InvalidPayload();
