...
contract CbCrossChainTest is CbStructs, Test {
  ...
  /// Encode a PayablePayload for handleReceiveFinalizedMessage.
  function _encodeCctpPayload(bytes32 payableId_, uint8 actionType_, uint64 nonce_)
    internal
    pure
    returns (bytes memory)
  {
    return _encodePayload(payableId_, actionType_, nonce_);
  }
  ...
  function testReceivePayableUpdateViaWormholeSuccessCreate() public {
    ...
    emit ReceivedPayableUpdateViaWormhole(foreignPayableId, foreignCbChainId, 1, vaaHash);
    chainbills.receivePayableUpdateViaWormhole(bytes(''));
  ...
  function testReceivePayableUpdateViaWormholeConsumedMessageRecorded() public {
    ...
    assertEq(cbGetters.getWormholeStats().consumedWormholeMessagesCount, 1);
  ...
  // -------------------------------------------------------------------------
  // handleReceiveFinalizedMessage (Circle CCTP v2 callback)
  // -------------------------------------------------------------------------

  function testHandleReceiveMessageRevertsForNonTransmitter() public {
    bytes memory payload = _encodeCctpPayload(keccak256('fp'), 1, 1);
    vm.prank(makeAddr('random'));
    vm.expectRevert(CircleTransmitterOnly.selector);
    chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, foreignEmitter, 2000, payload);
  }
  ...
  function testHandleReceiveMessageRevertsOnUnregisteredDomain() public {
    ...
    chainbills.handleReceiveFinalizedMessage(99, foreignEmitter, 2000, payload);
  }
  ...
  function testHandleReceiveMessageRevertsOnSenderMismatch() public {
    ...
    chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, wrongSender, 2000, payload);
  }
  ...
  function testHandleReceiveMessageRevertsOnStaleNonce() public {
    ...
    bytes memory payload5 = _encodeCctpPayload(foreignPayableId, 1, 5);
    bytes memory payload4 = _encodeCctpPayload(foreignPayableId, 2, 4);

    // Deliver nonce=5 first.
    vm.prank(address(mockCircleTransmitter));
    chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, foreignEmitter, 2000, payload5);

    // Deliver nonce=4 (stale).
    vm.prank(address(mockCircleTransmitter));
    vm.expectRevert(StalePayableUpdateNonce.selector);
    chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, foreignEmitter, 2000, payload4);
  }

  function testHandleReceiveMessageSuccessCreate() public {
    bytes32 foreignPayableId = keccak256('fp-cctp-create');
    bytes memory payload = _encodeCctpPayload(foreignPayableId, 1, 1);
  ...
  function testHandleReceiveMessageSuccessReopen() public {
    ...
    bytes memory payloadCreate = _encodeCctpPayload(foreignPayableId, 1, 1);
    bytes memory payloadClose = _encodeCctpPayload(foreignPayableId, 2, 2);
    bytes memory payloadReopen = _encodeCctpPayload(foreignPayableId, 3, 3);

    // Create.
    vm.prank(address(mockCircleTransmitter));
    chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, foreignEmitter, 2000, payloadCreate);

    // Close.
    vm.prank(address(mockCircleTransmitter));
    chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, foreignEmitter, 2000, payloadClose);
    assertTrue(cbGetters.getForeignPayable(foreignPayableId).isClosed);

    // Reopen.
    vm.prank(address(mockCircleTransmitter));
    chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, foreignEmitter, 2000, payloadReopen);
  ...
  function testHandleReceiveMessageNoncePreventsDuplicates() public {
    ...
    bytes memory payload10 = _encodeCctpPayload(foreignPayableId, 1, 10);
    bytes memory payload10again = _encodeCctpPayload(foreignPayableId, 1, 10);

    // First delivery.
    vm.prank(address(mockCircleTransmitter));
    chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, foreignEmitter, 2000, payload10);

    // Same nonce — stale.
    vm.prank(address(mockCircleTransmitter));
    vm.expectRevert(StalePayableUpdateNonce.selector);
    chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, foreignEmitter, 2000, payload10again);
  }
  ...
  function testWormholeAndCctpShareNonces() public {
    ...
    bytes memory cctpStale = _encodeCctpPayload(foreignPayableId, 2, 5);
    bytes memory cctpFresh = _encodeCctpPayload(foreignPayableId, 2, 6);
    ...
    vm.prank(address(mockCircleTransmitter));
    vm.expectRevert(StalePayableUpdateNonce.selector);
    chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, foreignEmitter, 2000, cctpStale);

    // Deliver nonce=6 via CCTP — must succeed.
    vm.prank(address(mockCircleTransmitter));
    chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, foreignEmitter, 2000, cctpFresh);
  }
  ...
  function _setupPaymentVm(bytes32 localPayableId, uint64, bytes32 vaaHash) internal returns (bytes memory circleMsg) {
    ...
    circleMsg = _buildCircleBurnMessage(
  ...
  function testReceiveForeignPaymentRevertsOnCircleSourceDomainMismatch() public {
    ...
    bytes memory circleMsg = _buildCircleBurnMessage(
      uint32(99), uint32(0), bytes32(0), foreignEmitter, toWormholeFormat(address(chainbills)), 1e6
    );
  ...
  function testReceiveForeignPaymentRevertsOnCircleTargetDomainMismatch() public {
    ...
    bytes memory circleMsg = _buildCircleBurnMessage(
      foreignCircleDomain, uint32(99), bytes32(0), foreignEmitter, toWormholeFormat(address(chainbills)), 1e6
    );
  ...
  function testReceiveForeignPaymentRevertsOnCircleSenderMismatch() public {
    ...
    bytes memory circleMsg = _buildCircleBurnMessage(
      foreignCircleDomain, uint32(0), bytes32(0), wrongSender, toWormholeFormat(address(chainbills)), 1e6
    );
  ...
  function testHandleReceiveMessageRevertsOnUnknownTypeByte() public {
    // 0x03 is not a valid msgType (1 or 2).
    bytes memory payload = abi.encodePacked(uint8(3), bytes('garbage'));
    vm.prank(address(mockCircleTransmitter));
    vm.expectRevert(InvalidPayload.selector);
    chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, foreignEmitter, 2000, payload);
  }

  function testReceiveForeignPaymentRevertsOnCircleAmountMismatch() public {
    vm.prank(host);
    (bytes32 localPayableId,) = chainbills.createPayable(new TokenAndAmount[](0), false);
  ...
  function testHandleReceiveMessageReturnsTrueForType2NoOp() public {
    // Type 0x02 is PaymentPayload companion data message.
    // It should be a no-op that returns true.
    bytes memory payload = abi.encodePacked(uint8(2), bytes('companion-data'));
    vm.prank(address(mockCircleTransmitter));
    bool ok = chainbills.handleReceiveFinalizedMessage(foreignCircleDomain, foreignEmitter, 2000, payload);
    assertTrue(ok);
  }
  ...
  function testReceiveForeignPaymentCctpOnlyRevertsOnInvalidDomain() public {
    ...
    bytes memory payBody = _buildPaymentPayloadBody(localPayableId, 1e6, makeAddr('cctp-domain-payer'), 0);
  ...
  function testReceiveForeignPaymentCctpOnlyRevertsOnSenderMismatch() public {
    ...
    bytes memory payBody = _buildPaymentPayloadBody(localPayableId, 1e6, makeAddr('cctp-sender-payer'), 0);
  ...
  function testReceiveForeignPaymentCctpOnlyRevertsOnRecipientMismatch() public {
    ...
    bytes memory payBody = _buildPaymentPayloadBody(localPayableId, 1e6, makeAddr('cctp-recip-payer'), 0);
  ...
  function testReceiveForeignPaymentCctpOnlyRevertsOnDataMessageFailed() public {
    ...
    bytes memory payBody = _buildPaymentPayloadBody(localPayableId, 1e6, makeAddr('cctp-fail-payer'), 0);
    bytes memory dataMsg =
  ...
  function testReceiveForeignPaymentCctpOnlyRevertsOnAmountMismatch() public {
    ...
    bytes memory payBody = _buildPaymentPayloadBody(localPayableId, 1e6, makeAddr('cctp-amt-payer'), 0);
    bytes memory dataMsg =
      _buildCircleDataMessage(foreignCircleDomain, foreignEmitter, toWormholeFormat(address(chainbills)), payBody);
  ...
  function testReceiveForeignPaymentCctpOnlyRevertsOnBurnNonceAlreadyConsumed() public {
    ...
    bytes memory payBody = _buildPaymentPayloadBody(localPayableId, 1e6, makeAddr('cctp-nonce-payer'), 77);
    bytes memory dataMsg =
      _buildCircleDataMessage(foreignCircleDomain, foreignEmitter, toWormholeFormat(address(chainbills)), payBody);
    bytes memory circleMsg = _buildCircleBurnMessage(
  ...
  function testReceiveForeignPaymentCctpOnlySuccess() public {
    ...
    bytes memory payBody = _buildPaymentPayloadBody(localPayableId, 1e6, makeAddr('cctp-only-payer'), 88);
    bytes memory dataMsg =
      _buildCircleDataMessage(foreignCircleDomain, foreignEmitter, toWormholeFormat(address(chainbills)), payBody);
    bytes memory circleMsg = _buildCircleBurnMessage(
  ...
  function testReceiveForeignPaymentCctpOnlyWithAutoWithdraw() public {
    ...
    bytes memory payBody = _buildPaymentPayloadBody(localPayableId, 1e6, makeAddr('cctp-only-autow-payer'), 99);
    bytes memory dataMsg =
      _buildCircleDataMessage(foreignCircleDomain, foreignEmitter, toWormholeFormat(address(chainbills)), payBody);
    bytes memory circleMsg = _buildCircleBurnMessage(
  ...
  function testPayForeignWithCirclePublishesViaWormhole() public {
    // Protocol 1 = WORMHOLE → supportsWormhole(foreignCbChainId) returns true.
    vm.prank(owner);
