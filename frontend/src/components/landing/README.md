...
| `CrossChainRoute.vue`    | A horizontal source-chain → burn → attest → relay → destination-chain diagram. A local stand-in for the shared component brief 03 (`src/components/tx/`) will eventually own — swap it in once that folder exists.                                                   |
