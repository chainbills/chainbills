 ...
 *
 * Columns: gross amount (reconstructed), fee (estimated), net amount
 * (on-chain), payable, host, timestamp, receipt link.
 *
 * The on-chain withdrawal record stores only the net amount. Gross and fee are
 * reconstructed from the current `withdrawalFeePercentage` chain config and
 * labelled "estimated" in the UI, as the brief requires.
 *
 * Props:
 ...
 *  - `withdrawalFeePercentage`: basis points (from chain stats), used to
 *    reconstruct gross from net. Pass 0 when not yet loaded.
 ...
 * Usage:
 * ```vue
 * <ScanWithdrawalsTable :chain-name="null" network-type="testnet" :page="0" :page-size="20" :withdrawal-fee-percentage="200" />
 * ```
 */
...
const props = withDefaults(defineProps<{
  ...
  pageSize: number;
  /**
   * Withdrawal fee in basis points from `getConfig().withdrawalFeePercentage`.
   * Used to reconstruct gross from net. Label the computed values "est." in the UI.
   */
  withdrawalFeePercentage?: number;
  /** Optional substring filter applied to the loaded window. */
  textFilter?: string;
}>(), { withdrawalFeePercentage: 0, textFilter: '' });
...
const columns: ColumnDef[] = [
  { label: 'Net amount', key: 'amount', slotName: 'net' },
  { label: 'Fee (est.)', key: 'amount', slotName: 'fee' },
  { label: 'Gross (est.)', key: 'amount', slotName: 'gross' },
...
/**
 * Reconstructs the gross amount from the net (stored on-chain) using the
 * current `withdrawalFeePercentage` in basis points. The fee is deducted
 * from the gross to give the net, so: gross = net * 10000 / (10000 - fee_bps).
 * Integer maths on bigint to avoid precision loss.
 */
const reconstructGross = (netBigInt: bigint): bigint => {
  const feeBps = BigInt(props.withdrawalFeePercentage);
  if (feeBps === 0n) return netBigInt;
  // gross = net * 10000 / (10000 - feeBps)
  return (netBigInt * 10000n) / (10000n - feeBps);
};
...
<template>
  ...
  >
    ...
    <!-- Fee (estimated from current fee config) -->
    <template #cell-fee="{ row }">
      <span class="tabular-nums text-sm text-muted">
        {{ formatAmt(reconstructGross(BigInt(row.amount ?? 0)) - BigInt(row.amount ?? 0)) }}
    ...
    <template #cell-gross="{ row }">
      <span class="tabular-nums text-sm text-muted">
        {{ formatAmt(reconstructGross(BigInt(row.amount ?? 0))) }}
        <span class="text-[10px]">est.</span>
      </span>
    </template>
    ...
    <template #row-detail="{ row }">
      <dl class="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
        ...
        <dd class="tabular-nums">{{ formatAmt(reconstructGross(BigInt(row.amount ?? 0)) - BigInt(row.amount ?? 0)) }}</dd>
        <dt class="text-muted">Gross (est.)</dt>
        <dd class="tabular-nums">{{ formatAmt(reconstructGross(BigInt(row.amount ?? 0))) }}</dd>
