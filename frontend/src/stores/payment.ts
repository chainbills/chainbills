...
export const usePaymentStore = defineStore('payment', () => {
  ...
  const pay = async (
    ...
    try {
      ...
      const { account: thisProgramTokenAccount, exists: thisProgramTAExists } =
        await getATAAndExists(mint, thisProgram);
      ...
      call.preInstructions([
        ...
        ...(!payerTAExists
          ...
          : []),
        ...(!thisProgramTAExists
          ? [createATAInstruction(thisProgramTokenAccount, thisProgram, mint)]
          : []),
      ]);
      ...
      await server.madePayment(payment.toBase58(), email);
      toast.add({
        severity: 'success',
