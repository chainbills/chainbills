...
export const b64 = {
  ...
  encode: (input: string | Uint8Array) => base64.encode(typeof input === 'string' ? bytes.encode(input) : input),
};
...
export const b58 = {
  ...
  encode: (input: string | Uint8Array) => base58.encode(typeof input === 'string' ? bytes.encode(input) : input),
};
...
export const bignum = {
  ...
  toNumber: (input: bigint) => {
    if (input > BigInt(Number.MAX_SAFE_INTEGER)) throw new Error(`Invalid cast: ${input} exceeds MAX_SAFE_INTEGER`);

    return Number(input);
  },
  /** safe cast from number to bigint */
  toBigInt: (input: number) => {
    if (input > Number.MAX_SAFE_INTEGER) throw new Error(`Invalid cast: ${input} exceeds MAX_SAFE_INTEGER`);
...
export const bytes = {
  ...
  equals: (lhs: Uint8Array, rhs: Uint8Array): boolean => lhs.length === rhs.length && lhs.every((v, i) => v === rhs[i]),
