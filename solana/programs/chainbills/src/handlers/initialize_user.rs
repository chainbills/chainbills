...
pub fn initialize_user_handler(ctx: Context<InitializeUser>) -> Result<()> {
  ...
  // Initialize the chain user address account.
  let chain_user_address = ctx.accounts.chain_user_address.as_mut();
  chain_user_address.user_address = ctx.accounts.signer.key();
